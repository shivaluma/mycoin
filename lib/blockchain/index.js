  ...
  checkBlock(newBlock, previousBlock, referenceBlockchain = this.blocks) {
    ...
    if (previousBlock.index + 1 !== newBlock.index) {
      ...
      throw new BlockAssertionError(
        `Invalid index: expected '${previousBlock.index + 1}' got '${newBlock.index}'`
      );
    ...
    let sumOfInputsAmount =
      R.sum(
        R.flatten(
          R.map(
            R.compose(R.map(R.prop('amount')), R.prop('inputs'), R.prop('data')),
  ...
  getUnspentTransactionsForAddress(address) {
    ...
    R.forEach((txOutput) => {
      if (
        !R.any(
          (txInput) =>
            txInput.transaction == txOutput.transaction && txInput.index == txOutput.index,
          txInputs
        )
      ) {
